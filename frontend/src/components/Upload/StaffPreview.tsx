...
export default function StaffPreview({
  ...
  useEffect(() => {
    ...
    // Initialize with staff-only mode to show standard musical notation
    const renderer = new AlphaTabRenderer(containerEl, 'staff-only')
    ...
    renderer.setOnError((err) => {
      console.error('AlphaTab staff preview error:', err)
      if (!isMounted) {
        return
      }
      setError(err.message || 'Failed to load staff notation preview')
      setIsLoading(false)
    })
    ...
    renderer
      ...
      .catch((err) => {
        console.error('Failed to load MusicXML for staff preview:', err)
        if (!isMounted) {
          return
        }
        setError('Could not load staff notation preview')
        setIsLoading(false)
      })
    ...
    <div className="space-y-3">
        ...
        {isLoading && (
          <div className="flex h-full items-center justify-center p-6 text-sm text-gray-600">
            <div className="flex flex-col items-center gap-2">
              ...
              <span>{loadingMessage || 'Loading staff notation preview…'}</span>
            </div>
          </div>
        )}
        {error && (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-2 flex items-center gap-2">
          <label htmlFor="staff-instrument" className="text-sm font-medium text-gray-700">
            Instrument
          </label>
          <select
            id="staff-instrument"
